...
public class Jww2SqlInterceptor implements Interceptor {
    private final Logger logger = LoggerFactory.getLogger(this.getClass());

    @Override
    public Object intercept(Invocation invocation) throws Throwable {
        try {
            String dbName = DynamicDataSourceContextHolder.getDataSourceLookupKey();
            if ("jww2".equals(dbName)) {
                ...
                ThreadLocalUtil.set("logsql", sqlList);
                //log.debug(sql);
            }
        } catch (Exception e) {
